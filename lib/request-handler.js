...
RequestHandler.prototype.render = function (view, data) {
  ...
  this.application.options.templateEngine.on('render', function (output) {
    self.write(output);
  });
    
  this.application.options.templateEngine.render(view, data);
};
...
StaticRequestHandler.prototype.serveFile = function (filePath) {
  ...
  filePath = path.resolve(path.join(this.application.options.static.path, filePath));
  ...
  function existsCb (exists) {
    ...
    Fiber(function() {
      var fileStat = stat(filePath).wait();
